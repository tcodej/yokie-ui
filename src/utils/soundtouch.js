...
WebAudioBufferSource.prototype = {
    extract: function(target, numFrames, position) {
        ...
        var r;
        if (this.buffer.numberOfChannels > 1) r = this.buffer.getChannelData(1);
        for (var i = 0; i < numFrames; i++) {
            target[i * 2] = l[i + position];
            if (this.buffer.numberOfChannels > 1) target[i * 2 + 1] = r[i + position];
        }
...
export function PitchShifter (ctx, buffer, bufSize) {
